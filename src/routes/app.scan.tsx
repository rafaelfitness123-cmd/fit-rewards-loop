...
export const Route = createFileRoute("/app/scan")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Escaneie o QR Code da academia e veja seu resumo: pontos, sequência e missões.",
      },
      { property: "og:title", content: "Check-in por QR Code — PulseFit" },
      {
        property: "og:description",
        content: "Registre entrada e saída do treino e acompanhe pontos e missões.",
      },
    ],
  }),
  component: Scan,
...
function Scan() {
  ...
  const [dados] = useStore(() => {
    const s = getSessao();
    const cid = s?.tipo === "cliente" ? s.clienteId : undefined;
    if (!cid) return null;
    const treinos = treinosDe(cid);
    return {
      pontos: saldoDe(cid),
      streak: sequenciaAtual(cid),
      ...
      missoes: getMissoes()
        ...
        .map((m) => ({ m, p: progressoDaMissao(cid, m) }))
    ...
    <div className="space-y-5">
      <section className="hero-surface p-5">
        ...
        <div className="mt-5 grid grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-2xl bg-background/40 p-3">
            ...
            <p className="text-[11px] text-muted-foreground">de {dados.totalClientes}</p>
          </div>
        </div>
      ...
      {aberto && (
        <p className="text-center text-xs text-muted-foreground">
          Tempo atual:{" "}
          {formatarDuracao(duracaoMinutos({ ...aberto, saida: new Date().toISOString() }))}
        </p>
      )}
